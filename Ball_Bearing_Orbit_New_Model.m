...
% Initial position & velocity vector of ball and races center
    ...
    for n=1:N
        ...
        x_b(n,1) = (Ri+Rb)*cos((2*pi/N)*(n-1)+0.5*pi/N);
        y_b(n,1) = (Ri+Rb)*sin((2*pi/N)*(n-1)+0.5*pi/N);
...
for i=1:maxItr
    ...
    kox2_1 = ts*fun_race_outer(Mo,K_sd,c_sd,F_outer(1),x_o(i),u_o(i));
    kox1_2 = ts*fun_distance(u_o(i)+0.5*kox2_1);
    kox2_2 = ts*fun_race_outer(Mo,K_sd,c_sd,F_outer(1),x_o(i)+0.5*kox1_1,u_o(i)+0.5*kox2_1);
    kox1_3 = ts*fun_distance(u_o(i)+0.5*kox2_2);
    kox2_3 = ts*fun_race_outer(Mo,K_sd,c_sd,F_outer(1),x_o(i)+0.5*kox1_2,u_o(i)+0.5*kox2_2);
    kox1_4 = ts*fun_distance(u_o(i)+kox2_3);
    kox2_4 = ts*fun_race_outer(Mo,K_sd,c_sd,F_outer(1),x_o(i)+kox1_3,u_o(i)+kox2_3);
    
    koy1_1 = ts*fun_distance(v_o(i));
    koy2_1 = ts*fun_race_outer(Mo,K_sd,c_sd,F_outer(2),y_o(i),v_o(i));
    koy1_2 = ts*fun_distance(v_o(i)+0.5*koy2_1);
    koy2_2 = ts*fun_race_outer(Mo,K_sd,c_sd,F_outer(2),y_o(i)+0.5*koy1_1,v_o(i)+0.5*koy2_1);
    koy1_3 = ts*fun_distance(v_o(i)+0.5*koy2_2);
    koy2_3 = ts*fun_race_outer(Mo,K_sd,c_sd,F_outer(2),y_o(i)+0.5*koy1_2,v_o(i)+0.5*koy2_2);
    koy1_4 = ts*fun_distance(v_o(i)+koy2_3);
    koy2_4 = ts*fun_race_outer(Mo,K_sd,c_sd,F_outer(2),y_o(i)+koy1_3,v_o(i)+koy2_3);
...
title(str)
grid on
saveas(fig_3,str,'png')

fig_4=figure(4);
plot(t,x_o)
str=sprintf("X_o vs T for %dN eccentric loading",Fo);
title(str)
grid on
saveas(fig_4,str,'png')

fig_5=figure(5);
plot(t,y_o);
str=sprintf("Y_o vs T for %dN eccentric loading",Fo);
title(str)
grid on
saveas(fig_5,str,'png')

L=length(t); fs=(1/ts); f=(fs/L)*(0:L/2); pp=fft(x_o); pp=abs(pp);
pp=pp(1:L/2+1); pp=normalize(pp,'range');
fig_6=figure(6);
plot(f(1:L/1e4),pp(1:L/1e4));
frequency=round(Wi/(2*pi));
...
function a= fun_race_outer(M,k,c,F,displacement,velocity)
    a=(F-2*k*displacement-2*c*velocity)/M;
end
